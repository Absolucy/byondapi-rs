...
cfg_if::cfg_if! {
	...
	} else if #[cfg(not(target_arch = "x86"))] {
		#[link(name = "byondapi", kind = "static")]
	} else if #[cfg(not(any(target_os = "linux", target_os = "windows")))] {
...
pub use byond_rawbind::{
	s1c, s1cMAX, s1cMIN, s2c, s2cMAX, s2cMIN, s4c, s4cMAX, s4cMIN, s8c, u1c, u2c, u4c,
	u4cOrPointer, u8c, ByondValueData, ByondValueType, CByondValue, CByondValueList,
};
...
impl ByondApi {
	...
	pub unsafe fn Byond_CallGlobalProc(
		...
		self.internal
			.Byond_CallGlobalProc(name, arg.cast(), arg_count, result.cast())
	}
	...
	pub unsafe fn Byond_CallGlobalProcByStrId(
		...
		self.internal
			.Byond_CallGlobalProcByStrId(name, arg.cast(), arg_count, result.cast())
	}
