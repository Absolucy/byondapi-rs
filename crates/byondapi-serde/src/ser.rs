...
impl<'a> Serializer for &'a mut ByondSerializer {
	...
	fn serialize_bytes(self, v: &[u8]) -> Result<Self::Ok, Self::Error> {
		...
		for byte in v {
			list.push(&byte.to_byond()?)?;
		}
		list.to_byond().map_err(SerializeError::from)
	}
		...
		T: Serialize,
	{
		...
		let variant = variant.to_byond()?;
		let value = value.serialize(self)?;
		list.write_list_index_internal(&variant, &value)?;
		Ok(list)
	}

	#[inline]
	fn serialize_seq(self, _len: Option<usize>) -> Result<Self::SerializeSeq, Self::Error> {
		Ok(seq::ByondSeqSerializer::new(self))
	}
	...
	fn serialize_tuple_variant(
		...
		Ok(variant::ByondVariantSerializer::new(self, variant))
	}
