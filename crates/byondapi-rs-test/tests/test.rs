...
fn compile() {
	let dm_compiler = find_dm().expect(
		"To run this integration test you must place a copy of BYOND binaries in \
		 dm_project/byond/bin",
	);
...
fn run_dreamdaemon(tempdir: &TempDir) {
	let dream_daemon = find_dd().expect(
		"To run this integration test you must place a copy of BYOND binaries in \
		 dm_project/byond/bin",
	);
